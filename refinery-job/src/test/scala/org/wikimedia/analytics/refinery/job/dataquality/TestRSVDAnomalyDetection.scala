...
class TestRSVDAnomalyDetection extends FlatSpec with Matchers with DataFrameSuiteBase {
    ...
    it should "fill in gaps in incomplete time series" in {
        ...
        assert(filledTimeSeries(0) == ("2020-01-01", 1.0))
        assert(filledTimeSeries(1) == ("2020-01-02", 2.0))
        assert(filledTimeSeries(2) == ("2020-01-03", 0.0))
        assert(filledTimeSeries(3) == ("2020-01-04", 3.0))
        assert(filledTimeSeries(4) == ("2020-01-05", 0.0))
    }
