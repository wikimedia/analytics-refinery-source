...
class TestWikidataJsonParse extends FlatSpec with Matchers {

  lazy implicit val formats = DefaultFormats

  "Wikidata" should "work properly with examples entities" in {
    val entityStrings = Seq(
      """
      |{
      |  "pageid": 186,
      |  "ns": 0,
    ...
    val jsonEntities = entityStrings.map(WikidataJsonDumpConverter.parseEntity)
    jsonEntities.size should be(5)
    val entities = jsonEntities.map(new Entity(_))
