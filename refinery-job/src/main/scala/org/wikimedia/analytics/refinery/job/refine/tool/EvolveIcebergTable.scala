...
object EvolveIcebergTable extends ConfigHelper with TransformFunctionsConfigHelper with LogHelper {
    ...
    object Config {
        val usage: String = s"""
            ...
            |
            |    --location=<hdfs_path>
            |       HDFS path of the Iceberg table.
    ...
    def apply(
        ...
        new EvolveIcebergTable(EventSparkSchemaLoader.buildEventSchemaLoader(baseSchemaUris), spark)
    }
...
class EvolveIcebergTable(
    ...
    def evolveIcebergTableWithSchema(
        ...
        val sparkSchema = JsonSchemaSparkConverter.toDataType(jsonSchema).asInstanceOf[StructType]
