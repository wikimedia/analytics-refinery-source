...
/**
 * Case class representing SiteInfo for a specific wiki
 * Generated with a query similar to:
 ...
 *     and dbname = 'simplewiki'
 *   group by language, sitename, dbname, home_page, mw_version, case_setting;
 *
 * @param languageCode two letter language code
 * @param siteName also known as the "project family"
 * @param dbName name of the database
 * @param homePage each wiki has their own
 * @param mediaWikiVersion the version this wiki is running right now
 * @param caseSetting the case setting on this wiki (first-letter or case-sensitive)
    ...
    def getXML: String = {
        f"""|  <siteinfo>
            ...
            |    <namespaces>${namespaces.map( ns => {
                     val prefix = f"""      <namespace key="${ns.code}" case="${ns.caseSetting}""""
                     if (ns.name.isEmpty) {
                         prefix + " />"
                     } else {
