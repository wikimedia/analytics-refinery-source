  ...
  def fromRow(row: Row): MediawikiEvent =
    new MediawikiEvent(
      ...
      eventUserDetails = new MediawikiEventUserDetails(
        ...
        userRegistrationTimestamp = if (row.isNullAt(17)) None else Some(Timestamp.valueOf(row.getString(17))),
        userCreationTimestamp = if (row.isNullAt(18)) None else Some(Timestamp.valueOf(row.getString(18))),
        userFirstEditTimestamp = if (row.isNullAt(19)) None else Some(Timestamp.valueOf(row.getString(19))),
        userRevisionCount = if (row.isNullAt(20)) None else Some(row.getLong(20)),
        userSecondsSincePreviousRevision = if (row.isNullAt(21)) None else Some(row.getLong(21))
      ),
      pageDetails = new MediawikiEventPageDetails(
        pageId = if (row.isNullAt(22)) None else Some(row.getLong(22)),
        pageArtificialId = Option(row.getString(23)),
        pageTitleHistorical = Option(row.getString(24)),
        pageTitle = Option(row.getString(25)),
        pageNamespaceHistorical = if (row.isNullAt(26)) None else Some(row.getInt(26)),
        pageNamespaceIsContentHistorical = if (row.isNullAt(27)) None else Some(row.getBoolean(27)),
        pageNamespace = if (row.isNullAt(28)) None else Some(row.getInt(28)),
        pageNamespaceIsContent = if (row.isNullAt(29)) None else Some(row.getBoolean(29)),
        pageIsRedirect = if (row.isNullAt(30)) None else Some(row.getBoolean(30)),
        pageIsDeleted = if (row.isNullAt(31)) None else Some(row.getBoolean(31)),
        pageCreationTimestamp = if (row.isNullAt(32)) None else Some(Timestamp.valueOf(row.getString(32))),
        pageRevisionCount = if (row.isNullAt(33)) None else Some(row.getLong(33)),
        pageSecondsSincePreviousRevision = if (row.isNullAt(34)) None else Some(row.getLong(34))
      ),
      userDetails = new MediawikiEventUserDetails(
        userId = if (row.isNullAt(35)) None else Some(row.getLong(35)),
        userTextHistorical = Option(row.getString(36)),
        userText = Option(row.getString(37)),
        userBlocksHistorical = Option(row.getSeq[String](38)),
        userBlocks = Option(row.getSeq[String](39)),
        userGroupsHistorical = Option(row.getSeq[String](40)),
        userGroups = Option(row.getSeq[String](41)),
        userIsCreatedBySelf = if (row.isNullAt(42)) None else Some(row.getBoolean(42)),
        userIsCreatedBySystem = if (row.isNullAt(43)) None else Some(row.getBoolean(43)),
        userIsCreatedByPeer = if (row.isNullAt(44)) None else Some(row.getBoolean(44)),
        userIsAnonymous = if (row.isNullAt(45)) None else Some(row.getBoolean(45)),
        userIsBotByName = if (row.isNullAt(46)) None else Some(row.getBoolean(46)),
        userRegistrationTimestamp = if (row.isNullAt(47)) None else Some(Timestamp.valueOf(row.getString(47))),
        userCreationTimestamp = if (row.isNullAt(48)) None else Some(Timestamp.valueOf(row.getString(48))),
        userFirstEditTimestamp = if (row.isNullAt(49)) None else Some(Timestamp.valueOf(row.getString(49)))
      ...
      revisionDetails = new MediawikiEventRevisionDetails(
        revId = if (row.isNullAt(50)) None else Some(row.getLong(50)),
        revParentId = if (row.isNullAt(51)) None else Some(row.getLong(51)),
        revMinorEdit = if (row.isNullAt(52)) None else Some(row.getBoolean(52)),
        revDeletedParts = Option(row.getSeq[String](53)),
        revDeletedPartsAreSuppressed = if (row.isNullAt(54)) None else Some(row.getBoolean(54)),
        revTextBytes = if (row.isNullAt(55)) None else Some(row.getLong(55)),
        revTextBytesDiff = if (row.isNullAt(56)) None else Some(row.getLong(56)),
        revTextSha1 = Option(row.getString(57)),
        revContentModel = Option(row.getString(58)),
        revContentFormat = Option(row.getString(59)),
        revIsDeletedByPageDeletion = if (row.isNullAt(60)) None else Some(row.getBoolean(60)),
        revDeletedByPageDeletionTimestamp = if (row.isNullAt(61)) None else Some(Timestamp.valueOf(row.getString(61))),
        revIsIdentityReverted = if (row.isNullAt(62)) None else Some(row.getBoolean(62)),
        revFirstIdentityRevertingRevisionId = if (row.isNullAt(63)) None else Some(row.getLong(63)),
        revSecondsToIdentityRevert = if (row.isNullAt(64)) None else Some(row.getLong(64)),
        revIsIdentityRevert = if (row.isNullAt(65)) None else Some(row.getBoolean(65)),
        revTags = Option(row.getSeq[String](66))
      )
    )
