...
object PageState {
  ...
  def fromRow(row: Row): PageState = PageState(
    ...
    causedByUserCentralId = getOptLong(row, 17),
    causedByAnonymousUser = getOptBoolean(row, 18),
    causedByTemporaryUser = getOptBoolean(row, 19),
    causedByPermanentUser = getOptBoolean(row, 20),
    causedByUserText = getOptString(row, 21),
    inferredFrom = getOptString(row, 22),
    sourceLogId = getOptLong(row, 23),
    sourceLogComment = getOptString(row, 24),
    sourceLogParams = getOptMap[String, String](row, 25),
    pageFirstState = row.getBoolean(26)
  )
