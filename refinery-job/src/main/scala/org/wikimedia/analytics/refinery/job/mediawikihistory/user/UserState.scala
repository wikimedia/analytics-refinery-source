...
object UserState {
  ...
  def fromRow(row: Row): UserState = UserState(
      ...
      userCreationTimestamp = if (row.isNullAt(9)) None else Some(Timestamp.valueOf(row.getString(9))),
      //userCreationTimestamp = if (row.isNullAt(9)) None else Some(row.getTimestamp(9)),
      userFirstEditTimestamp = if (row.isNullAt(10)) None else Some(Timestamp.valueOf(row.getString(10))),
      //userFirstEditTimestamp = if (row.isNullAt(10)) None else Some(row.getTimestamp(10)),
      createdBySelf = row.getBoolean(11),
      createdBySystem = row.getBoolean(12),
      createdByPeer = row.getBoolean(13),
      anonymous = row.getBoolean(14),
      botByName = row.getBoolean(15),
      startTimestamp = if (row.isNullAt(16)) None else Some(Timestamp.valueOf(row.getString(16))),
      //startTimestamp = if (row.isNullAt(16)) None else Some(row.getTimestamp(16)),
      endTimestamp = if (row.isNullAt(17)) None else Some(Timestamp.valueOf(row.getString(17))),
      //endTimestamp = if (row.isNullAt(17)) None else Some(row.getTimestamp(17)),
      causedByEventType = row.getString(18),
      causedByUserId = if (row.isNullAt(19)) None else Some(row.getLong(19)),
      causedByUserText = Option(row.getString(20)),
      causedByBlockExpiration = Option(row.getString(21)),
      inferredFrom = Option(row.getString(22)),
      sourceLogId = if (row.isNullAt(23)) None else Some(row.getLong(23)),
      sourceLogComment = Option(row.getString(24)),
      sourceLogParams = Option(row.getMap[String, String](25)).map(_.toMap)
  )
