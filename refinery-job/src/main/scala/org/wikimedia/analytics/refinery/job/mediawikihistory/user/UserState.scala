...
object UserState {
  ...
  def fromRow(row: Row): UserState = UserState(
      ...
      isAnonymous = row.getBoolean(16),
      isTemporary = row.getBoolean(17),
      isPermanent = row.getBoolean(18),
      startTimestamp = getOptTimestamp(row, 19),
      //startTimestamp = if (row.isNullAt(19)) None else Some(row.getTimestamp(19)),
      endTimestamp = getOptTimestamp(row, 20),
      //endTimestamp = if (row.isNullAt(20)) None else Some(row.getTimestamp(20)),
      causedByEventType = row.getString(21),
      causedByUserId = getOptLong(row, 22),
      causedByAnonymousUser = getOptBoolean(row, 23),
      causedByTemporaryUser = getOptBoolean(row, 24),
      causedByPermanentUser = getOptBoolean(row, 25),
      causedByUserText = getOptString(row, 26),
      causedByBlockExpiration = getOptString(row, 27),
      inferredFrom = getOptString(row, 28),
      sourceLogId = getOptLong(row, 29),
      sourceLogComment = getOptString(row, 30),
      sourceLogParams = getOptMap[String, String](row, 31)
  )
