package org.wikimedia.analytics.refinery.job.lite
...
object ProduceCanaryEvents extends ConfigHelper {
    val httpClientTimeoutInSeconds: Int = 10

    lazy val log: Logger = LogManager.getLogger(this.getClass.getName.split('.').last.split('$').last)
