...
object MWHistorySnapshotMerger {
  ...
  def run(spark: SparkSession, p: Params): Unit = {
    ...
    val insertSQL = buildInsertSQL(p)
...
WHERE s.snapshot     = '${p.snapshot}'
  ...
  DISTRIBUTE BY date_trunc('MONTH', event_timestamp)
  SORT BY wiki_id, page_id, event_timestamp"""
  }

  /**
   * Returns an INSERT that adds fresh source='snapshot' rows for page, revisions, and user entities.
   * Uses delete-then-insert semantics (no stable MERGE key in wmf.mediawiki_history for
   * these entities).
   *
   * No deduplication is applied here, because the online deduplication needed is
   * done upstream by DenormalizedRunner for wmf.mediawiki_history. For page
  ...
  def buildInsertSQL(p: Params): String =
    s"""INSERT INTO ${p.ref} (
  source,
  wiki_id,
