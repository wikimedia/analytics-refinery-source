...
class TestTableSchemaManager extends FlatSpec
    ...
    "hiveGetDDLStatements" should "generate statements to create a table" in {
        ...
        HiveTableSchemaManager.hiveGetDDLStatements(spark, newSchema, "db.table", "location", Seq("day")) should equal(Seq(
    ...
    it should "generate statements to create a table partitioned by datacenter and time" in {
        ...
        val partitions = Seq("datacenter", "year", "month", "day", "hour")
        HiveTableSchemaManager.hiveGetDDLStatements(spark, newSchema, "db.table", "location", partitions) should equal(Seq(
            """CREATE EXTERNAL TABLE db.table (
              |`wiki` STRING
              |)
              |PARTITIONED BY (
    ...
    it should "not generate statements to update a column to string, in this version of the code" in {
        ...
        HiveTableSchemaManager.hiveGetDDLStatements(spark, newSchema, "db.table", "location", Seq.empty) should equal(Seq())
    }
