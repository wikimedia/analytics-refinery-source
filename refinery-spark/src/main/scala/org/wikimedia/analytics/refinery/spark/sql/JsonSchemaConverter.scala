...
object JsonSchemaConverter {
    ...
    private def buildDataType(jsonSchema: JsonNode, fieldName: String): DataType = {
        if (jsonSchema == null ||  jsonSchema.isNull)
            throw new IllegalArgumentException("Schema node shouldn't be null when building DataType")
        if (!jsonSchema.isObject)
            throw new IllegalArgumentException("Schema node should be an object when building DataType")

        val schemaTypeNode = jsonSchema.get(typeField)
        if (schemaTypeNode == null || !schemaTypeNode.isTextual)
            throw new IllegalArgumentException("Schema must contain 'type' field")
    ...
    private def buildStructField(jsonSchema: JsonNode, fieldName: String, required: Boolean): StructField = {
        ...
        val field = StructField(fieldName, datatype, nullable = !required)
    ...
    private def buildStructType(jsonSchema: JsonNode, fieldName: String = "root"): StructType = {
        ...
        if (properties == null || !properties.isObject)
            throw new IllegalArgumentException(fieldName + " struct schema's \"properties\" is not an object.")
        val requiredFieldList = jsonSchema.get(requiredField)
        if (requiredFieldList != null && !requiredFieldList.isArray)
            throw new IllegalArgumentException(fieldName + " struct schema's \"required\" is not an array.")
        val requiredFieldSet = Try(requiredFieldList.elements().asScala.map(_.asText()).toSet).getOrElse(Set.empty)
