...
/** Exports a Deequ validation suite (e.g. a list or data validation check results) to
 * the a SparkDataFrame or text. This can be used for alerting and logging.
 *
 *
 * @param constraints
 * @param partition
 * @param runId
 */
class DeequVerificationSuiteToDataQualityAlerts(spark: SparkSession)(constraints: Seq[ConstraintResult],
    ...
    val outputSchema = StructType(Seq(
        ...
        StructField(ColumnNames.ConstraintId, StringType)
          .withComment("AWS Deequ validation suite: identified of this constraint type." +
    ...
    private val rows: Seq[Row] = for {
        ...
        Row(
            ...
            result.metric.get.value.getOrElse(null),
    ...
    // Default, system wide, iceberg table and HDFS output path. Users can change these settings.
    ...
    private val AlertOutputTable: String = "wmf.dataquality_alerts"
    private val AlertOutputPath: String = s"hdfs:////tmp/dataquality_alerts_${partition.tableName}_${partition.relativePath}"
        ...
        constraints
          ...
          .map { result =>
              s"""
                  ...
                  |value=${result.metric.get.value.get}
                  |message=${result.message.orNull}
...
object DeequVerificationSuiteToDataQualityAlerts {
    def apply(spark: SparkSession)(
               constraints: Seq[ConstraintResult],
               partition: HivePartition,
               runId: String
