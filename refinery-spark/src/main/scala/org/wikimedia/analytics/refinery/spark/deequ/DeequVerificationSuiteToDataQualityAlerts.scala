 ...
 * @param resultKey a Deequ Metrics resultKey used for adding tags and the date of the alert computation
 */
class DeequVerificationSuiteToDataQualityAlerts(spark: SparkSession)(verificationSuite: Map[Check, CheckResult],
    ...
    def this(spark: SparkSession,
        verificationSuite: Map[Check, CheckResult],
        partition: HivePartition,
        runId: String,
        resultKey: Option[ResultKey] = None) {
        this(spark)(verificationSuite, partition, runId, resultKey getOrElse ResultKey(System.currentTimeMillis()))
    }
    ...
    val outputSchema = StructType(Seq(
        ...
        StructField(ColumnNames.DatasetDate, LongType, nullable = true)
          .withComment("AWS Deequ resultKey: key insertion time."),
        StructField(
            ColumnNames.Tags,
            MapType(StringType, StringType, valueContainsNull = true),
            nullable = true
        ).withComment("AWS Deequ resultKey: key tags."),
    ))
...
object DeequVerificationSuiteToDataQualityAlerts {
    def apply(spark: SparkSession)(
               ...
               runId: String,
               resultKey: Option[ResultKey] = None
             ) = new DeequVerificationSuiteToDataQualityAlerts(spark, constraints, partition, runId, resultKey)
}