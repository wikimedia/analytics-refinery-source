 ...
 *
 *
 * As chain of responsability for fetching schemas is setup at this time this is the last link
 * and, as such, successor is null, so it does not delegate fetching of schemas.
 */
public class LocalRepoSchemaRegistry implements SchemaRegistry<Schema>, Handler<SchemaRegistry> {
    ...
    public SchemaDetails<Schema> getLatestSchemaByTopic(String topicName) {
        String schemaName = getSchemaNameFromTopic(topicName);
            ...
            if(latestRev == null) {
                throw new RuntimeException(String.format("Lastest schema for %s not found ",
                    topicName));
            }
